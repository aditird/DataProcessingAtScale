...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        for index,each_row in trips.iterrows():
                result = session.run("""
                    MERGE (pickup:Location { name: $pu_id })
                    MERGE (dropoff:Location { name: $do_id })
                    CREATE (pickup)-[trip:TRIP { distance: $dist, fare: $fare, pickup_dt: datetime($pu_dt), dropoff_dt: datetime($do_dt) }]->(dropoff)
                    """,
                    pu_id=int(each_row['PULocationID']),
                    do_id=int(each_row['DOLocationID']),
                    dist=float(each_row['trip_distance']),
                    fare=float(each_row['fare_amount']),
                    pu_dt=each_row['tpep_pickup_datetime'],
                    do_dt=each_row['tpep_dropoff_datetime'])
